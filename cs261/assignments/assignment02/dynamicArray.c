/* CS261- Assignment 2 - Dynamic Array*/
/* Name: Brian Wilkins
 * Date: 10/18/2015
...
void _dynArrSetCapacity(DynArr *v, int newCap)
{
	...
	//reinitializes array to newCap capacity
	initDynArr(v, newCap);

	//put the old data into the new array
	for(i = 0; i < oldSize; i++){
		v->data[i] = oldData[i];
	}
	//reset size back to old size
	v->size = oldSize;

	//deallocate the memory taken by the old data array
	free(oldData);
...
void addDynArr(DynArr *v, TYPE val)
{
	...
	if(v->size >= v->capacity){ //reached capacity, double capacity. increment v->size.
		_dynArrSetCapacity(v, (2 * v->capacity));
	}

	v->data[sizeDynArr(v)] = val; //set the last element to val
	v->size++;
...
void swapDynArr(DynArr *v, int i, int  j)
{
	...
	//put first value into temp variable
	TYPE temp = v->data[i];
	//swap the values
	putDynArr(v, i, getDynArr(v, j));
	//put temp value into second value's position
	putDynArr(v, j, temp);
}
